...
PGPASSWORD=${DB_OWNER_PASS} psql --echo-all -U ${DB_OWNER} postgres <<EOF

drop database if exists ${DB_NAME};

EOF

PGPASSWORD=${POSTGRES_PASSWORD} psql --echo-all -U ${POSTGRES_USER} postgres <<EOF
...
PGPASSWORD=${DB_OWNER_PASS} psql --echo-all -U ${DB_OWNER} ${DB_NAME} <<EOF
...
-- This line was added to enable DB_APP to create tables
GRANT ALL ON SCHEMA ${DB_SCHEMA} TO ${DB_APP};
...
PGPASSWORD=${POSTGRES_PASSWORD} psql --echo-all -U ${POSTGRES_USER} ${DB_NAME} <<EOF
